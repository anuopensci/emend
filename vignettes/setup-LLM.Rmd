...
`emend` by default requires [Ollama](#ollama) to be installed and running. The benefit of using Ollama is that it uses a local LLM and requires no internet access. However, there is some set up and minimum computing requirements (depending on the LLM selected). There is no payment required to use Ollama.
...
Pull a LLM, say `llama3.1:8b` (see list of models available [here](https://ollama.com/library)). We have used this model to demonstrate the result. 
